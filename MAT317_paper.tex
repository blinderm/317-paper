...
Notice that if a point $P$ is on a line $l$, then the reflection of $P$ over $l$ will have no effect. Regarding our point $P$ and line $l$ as their respective reflections, we then have that $f_l(P) = P$. FOR THE SAME REASONS I GUESS, we have that $M_l M_P M_l\inv = M_P$, whence $M_l M_P = M_P M_l$. We will first consider this in terms of matrices for lines not passing through the end $\specialend$. We have the following\footnote{$M_l M_P$ is a representative of $M_P M_l$, hence the $\lambda$}:
\begin{equation} 
...
of linear fractional transformations and let $GL_2(F)$ denote the group of invertible $2\times 2$ matrices with entries in $F$. Also define the subset
	...
	\]
	We argue that $F^*$ is a normal subgroup of $GL_2(F)$. DO MATH HERE. With this in mind, we now define the map $\varphi\colon A\rightarrow GL_2(F)/F^*$ by
	\[
		\varphi\left(z\mapsto \frac{az + b}{cz + d}\right) = \stanlftmat F^*
	\]
	We claim that $\varphi$ is a homomorphism. DO MORE MATH HERE. Now that we've established that $\varphi$ is a homomorphism, we now argue that $\varphi$ is surjective. MORE MATH. Next we identify the kernel of $\varphi$ in order to establish an isomorphism between $A/\ker(\varphi)$ and $GL_2(F)/F^*$ (TECHNICALLY DON'T NEED KERNEL TO IDENTIFY THE ISOMORPHISM, IT'S JUST NOTEWORTHY AND TELLS US WHEN WE CAN'T RECOVER A LFT). Finally by the First Isomorphism Theorem, we conclude that $A/\ker(\varphi)$ (INSERT WHATEVER KERNEL TURNS OUT TO BE) is isomorphic to $GL_2(F)/F^*$. 
