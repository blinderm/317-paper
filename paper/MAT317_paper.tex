...
=======
It should be explicitly noted that these matrices are used only to encode our transformations\footnote{``See appendix" or something}. That is, to ``apply" a transformation encoded the matrix in (2) to a point $z_0 \in \C$, we mean that we will apply a corresponding linear fractional transformation $f$ defined by (1) to the point $z_0$. Because all linear fractional transformations we will use are of the form given in (2), whenever we say ``matrix" we are referring to a $2 \times 2$ matrix. Finally, later we will be associating points with point reflections and lines with line reflections. In order to distinguish these geometric objects from their corresponding reflections, we will write $f_P(P_0)$ to mean the point reflection of the point $P_0$ over a point $P$ via the point reflection $f_P$ and $f_l(P_0)$ to be the line reflection of point $P_0$ over the line $l$ via the line reflection $f_l$. In a similar vein, we define $M_P$ and $M_l$ to be a matrix representation of a point reflection about $P$ and a line reflection about $l$ respectively. We will associate each point reflection with a unique point in the disk\footnote{How exactly does this ``unique association" work? }. Finally, we will discuss two types of line reflections: ones passing through the end $(\infty, \infty^2)$\footnote{see wherever} and ones not passing through this end. Throughout, the line to which we will refer passes through this end if and only if we explicitly specify that it does so (that is, if we simply say ``line reflection," then this line does not pass through the end $(\infty, \infty^2)$).
=======
...
>>>>>>> 52e36f012158514f7bbb5bca3354ea8bb8e6d10b
=======
It should be explicitly noted that these matrices are used only to encode our transformations\footnote{``See appendix" or something}. That is, to ``apply" a transformation encoded the matrix in (2) to a point $z_0 \in \C$, we mean that we will apply a corresponding linear fractional transformation $f$ defined by (1) to the point $z_0$. Because all linear fractional transformations we will use are of the form given in (2), whenever we say ``matrix" we are referring to a $2 \times 2$ matrix. Finally, later we will be associating points with point reflections and lines with line reflections. In order to distinguish these geometric objects from their corresponding reflections, we will write $f_P(P_0)$ to mean the point reflection of the point $P_0$ over a point $P$ via the point reflection $f_P$ and $f_l(P_0)$ to be the line reflection of point $P_0$ over the line $l$ via the line reflection $f_l$. In a similar vein, we define $M_P$ and $M_l$ to be a matrix representation of a point reflection about $P$ and a line reflection about $l$ respectively. We will associate each point reflection with a unique point in the disk\footnote{How exactly does this ``unique association" work? }. Finally, we will discuss two types of line reflections: ones passing through the end $(\infty, \infty^2)$\footnote{see wherever} and ones not passing through this end. Throughout, the line to which we will refer passes through this end if and only if we explicitly specify that it does so (that is, if we simply say ``line reflection," then this line does not pass through the end $(\infty, \infty^2)$).
>>>>>>> parent of 52e36f0... a bunch of scattered stuffi don't wanan lose
...
>>>>>>> 243c4467e9ae47aa0a8108a217087f2a76f5e40e

\[\]\wtflatex{What this Guggenheimer asshole did}
%\[\]\wtflatex{What this Guggenheimer fellow did}
...
blah blah blah we can see that line reflections are just point reflections with $\lambda = -1$ if we include that thing that has the fixed points? 


From these diagrams we notice two oddities. For one, 
...
Theorem: All line reflections defined by a point $l = (l_1, l_2)$ can be encoded as point reflections if we take the representative given by $\lambda = 1$

Proof: Suppose that we have a line reflection $f_l$ given by $l = (l_1, l_2)$. Then, by WHATEVER WE CALL IT we encode the corresponding reflection in the matrix $M_l = \stanlinenoendmat$. To show that $f_l$ can be encoded as a point reflection, notice that 
\[ -1 \cdot \stanlinenoendmat = \]
...
\newpage
\wtflatex{Appendix A1: You wanna draw? Let's fuckin' draw!}
%\wtflatex{Appendix A1: Drawing things!}
...
\noindent We begin by constructing a line segment between $A$ and the origin $O$ and in order to find the Euclidean distance, $d_E$, between points $A$ and $O$.
...
\noindent We then use the metric [INSERT THE METRIC YOU USE HERE] to find the hyperbolic distance, $d_H$, between $O$ and $A'$. We construct a circle of radius $d_H$ centered at $O$.

\begin{tikzpicture}
\draw (0,0) circle (2.2cm);
\draw (0,0) circle (1.41cm);
\draw (0,0) -- (2.2,2.2);
\draw[fill=black] (0,0) circle (0.05) node[left] {$O$};
\draw[fill=black] (2.2,2.2) circle (0.05) node[right] {$A$};
...
As mentioned, lines in the \poincare disk are given by the arcs of circles orthogonal to the 
\begin{tikzpicture}
\draw (0,0) circle (3cm);

%\draw (2.5,2.5) circle (3cm);
%\draw[fill=black] (2.5,2.5) circle (0.05) node[right] {$A$};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% VI.preA %%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage 
\wtflatex{Appendix A2: This thing is a field we promise}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% I.A %%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\wtflatex{Appendix A3: This shit is homomorphic we promise}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% I.A %%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\wtflatex{``Bibliography" (List of Sources)} 
...
\begin{enumerate}
	...
	\item Hartshorne's book for motivation: Robin Hartshorne, Geometry: Euclid And Beyond, Springer-Verlag New York, 2000, Chap, 7. 
	\item Cite this Geogebra worksheet: https://www.geogebra.org/material/show/id/7005
	\item Cite Geogebra tool (citing software): https://www.geogebra.org/home
\end{enumerate}
