...
\hspace{10mm} For centuries, mathematicians tried tirelessly to prove that Euclid's fifth postulate -- equivalent to the parallel postulate -- followed from the first four. It was not until the 19$^{\text{th}}$ century that Gauss and Bolyai showed that the the fifth postulate was independent from the others. This shocking result opened doors to the exploration of non-Euclidean geometric models, particularly those of hyperbolic geometry\footnote{TODO: Cite Hartshorne section 33.}.

To understand hyperbolic geometry we must first consider these five postulates, which axiomatize Euclidean geometry. The first four are as follows\footnote{TODO: Cite Hartshorne section 40.}:
...
\end{tabular}
\end{center}
As we can see in the Euclidean model, $m$ is the unique line through $P$ and parallel to $l$. On the other hand, in the hyperbolic model we have that $n_1$ and $n_2$ are distinct lines passing through $P$ and parallel to $l$. Further, there are infinitely many distinct lines with these properties.
