...
=======
Inspired by Euclidean geometry, we begin with the notion that both types of reflections should be of order two (that is, the composition of a transformation upon itself yields the identity transformation). 

\wtftheorem{Theorem} If $M$ is a matrix encoding some linear fractional transformation $f$, then $f$ is of order two if and only if $\Tr(M) = 0$ or $M \in \lambda I$ NOTATION???.

\wtftheorem{Proof} Let $M$ be a matrix that encodes a linear fractional transformation of order two. We then must have that $M^2 = \lambda I$ for some constant $\lambda \in F$\footnote{That is, $\lambda I$ is some member of the coset containing the identity.}, where $I$ is the usual $2 \times 2$ identity matrix. In this case, we want to have that 
>>>>>>> 4454004d55fffc49b04fb4b271bacb82fee2dd4f
	\[
...
for some $\lambda \in F$. Expanding this further, we see that we must have 
	...
	\]
Barring the trivial case in which $M$ is already of the form $\lambda I$, this can only be true if $\Tr(M) = 0$. 
