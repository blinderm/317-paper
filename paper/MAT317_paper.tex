...
The minor arc $AB$ of circle $C$ pictured above is a hyperbolic line in the \poincare disk realized by circle $O$. Further, any diameter $d$ of circle $O$ is a hyperbolic line in this disk, as $d$ is a generalized circle orthogonal to the boundary of the disk. 
...
However, there is no clear geometric motivation behind this choice anyway. In fact, the above examples in $\R$ demonstrate that this restriction is in fact entirely unnecessary. Consider the reflection of triangle $T_1$ over point $Y$. Triangle $T_1$ is defined by points contained in $D$. However, $T_1'$ (the result of reflecting $T_1$ over the point $(2, 1)$) is given by points that cannot be mapped from $D$ VERIFY AND REPHRASE. 
...
\begin{proof}
	...
	\begin{align*}
		...
		& = \varphi\left(\frac{(a_ga_f + b_gc_f)z + (a_gb_f + b_gd_f)}{(c_ga_f + d_gc_f)z + (c_gb_f + d_gd_f)}\right) & \text{(grouping terms for map)}\\
