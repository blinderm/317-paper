...
\end{equation}
We note that this encoding is well-defined and invertible\footnote{See Appendix \ref{appendixC}}.
...
To construct the point $A'$, we want to find the radius $r$ in Euclidean distance of the circle whose hyperbolic distance from $O$ is $d_E$; $A'$ lies on this circle of radius $r$. In other words, if $d_H$ is the hyperbolic metric and $d_H(O,A') = d_E(O,A)$, then we want to find the circle, with radius $r\in\R$ centered at $O$, that contains $A'$. The hyperbolic distance metric\footnote{[\cite{hyperbolic_metric}, p.~103]} is
\[
