...
After we have computed $f_X(A_2)$, we start the visualization process. First, we take $A_2$ and $f_X(A_2)$ and map them to their corresponding points in $\R^2$. In particular, let $A_{2_\R} = (3,5)$ and $f_X(A_2)_\R = (27/29, 5/29)$. Now, given the point $A_{2_\R}$ in $\R^2$, we apply the methodology from Appendix \ref{appendixA} to construct the accompanying point $A_{2_\R}'$ in the \poincare disk. 

\begin{enumerate}
	\item Compute the Euclidean distance from the origin to point $A$. In this case, we have $d_E((0,0),A_2) = \sqrt{34}$.
	\item Next, we want to find the radius of the circle that is equal to the Euclidean distance in hyperbolic distance. In particular, let $d_H(O,A') = d_E(O,A)$. Therefore, we have that
	\[
		d_H(O,A') = \sqrt{34} = \ln\frac{1 + r}{1 - r}.
	\]
	...
	\item Further, we construct the line from $(0,0)$ to $(5,3)$ which turns out to be $-3/5x + y = 0$. 
	\item Finally we compute the points of intersection between the circle and the line to find that the points $\{(-0.85,-.51),(0.85,.51)\}$. Since $A$ is positive in both coordinates, we want the point of intersection that is also positive in both coordinates. Therefore $A' = (0.85, .51)$.
\end{enumerate}
