...
\end{enumerate}
is replaced with its negation: ``Through an exterior point to a straight line we can construct an infinite number of parallels to that straight line.'' \cite{euclids_fifth}. In many contexts, a weaker (but equivalent) negation -- ``Through an exterior point to a straight line, we can construct at least two lines parallel to it.'' -- is used instead. However, since we primarily work with infinite fields, we find it more helpful to appeal to the former than the latter. \todo{[AB] fix}
...
The minor arc $AB$ of circle $\Delta$ that excludes points $A$ and $B$ \todo{[AB] say why later} pictured above defines the hyperbolic line through the pair of model points $(G,H)$ in the \poincare disk realized by circle $O$. Additionally the segment of the Euclidean line $l$ through $O$ that excludes $L$ and $K$  \todo{[AB] say why}  also defines hyperbolic line through the pair of model points $(E,F)$ in the \poincare disk model realized by $O$. 
...
\includegraphics[width=160mm]{../images/ends_example_1.png} \\
	\todo{this is off center!}
\end{center} 
%\fi


Now consider the following diagram above \todo{following above?}. Notice that the line $AB$ is parallel to the line $CD$ \todo{why is this parallel}. However, the rays $\overrightarrow{BA}$ and $\overrightarrow{DC}$ seem to meet at $\beta$ (note that $\beta$ is not defined within the model). We therefore define an end of the ray \todo{more importantly, of the geomtery} to be the equivalence class of all rays that meet at seem to `limit' \todo{rephrase} to the same point end of the circle $\Gamma$\footnote{[\cite{hartshorne}, Section 40]}. Furthermore, $\alpha$ and $\beta$ refer to different equivalence classes \todo{why}. An end therefore, is a way of discussing points on $\Gamma$ within the model without specifically including $\Gamma$. \todo{this will be important speial end} \todo{also rehash stuff about the special end}

	Further, as demonstrated intuitively by Figure \ref{fig:construction_hyperbolic_lines}, we claim that each hyperbolic line has exactly two distinct ends and that every pair of distinct ends defines a single hyperbolic line.
...
\todo{[AB] shouldn't this be in the intro?}
...
Now, let $F$ be some real, ordered field. The coefficients of our linear fractional transformations will come from $F$. We begin with a notion inspired by Euclidean geometry: that both point reflections and line reflections should be of order two\todo{ ([AB] as demonstrated above by the Euclidean reflections) awkward. will rephraase when decide where it goes} . That is, we want the composition of a transformation upon itself to yield the identity transformation. We first establish two lemmas. 
...
\begin{proof} 
	\todo{[LG] blend with appendix C}Suppose that we have two linear fractional transformations $f_1$ and $f_2$ given by $f(z) = \frac{a_1z+b_1}{c_1z+d_2}$ and $f_2(z) = \frac{a_2z+b_2}{c_2z+d_2}$ for each $z \in \C$. We then have by (2) that their respective matrix encodings are 
...
\end{align*}
Notice that this resultant matrix $M_{f_1}M_{f_2}$ encodes the linear fractional transformation given by $f_1 \circ f_2$, the composition of $f_1$ upon $f_2$.
\end{proof}	
...
\begin{proof}
	Because $f$ is of order two, by definition we must have that $f \circ f$ is the identity function. In terms of matrices, by Lemma 3.1 we must then have that $M_f^2$ yields the identity of $PGL(2\ttc F)$\footnote{See \ref{thm:PGL2F} in Appendix \ref{appendixC} for the definition of $PGL(2\ttc F)$ \todo{wordy?}}. That is, we must have that $M_f^2 = \lambda I$ for some $\lambda \in F$.
\end{proof}
...
Now, below are our diagrams, generated using the Geogebra\footnote{\cite{geogebra_software}} visual algebra tool and the corresponding user-made hyperbolic geometry worksheet\footnote{\cite{hyperbolic_worksheet}}.
...
\todo{[LG] karen's $\Gamma_{F^2}$ thing?}
...
To construct the point $A'$, we want to find the radius $r$ in Euclidean distance of the circle whose hyperbolic distance from $O$ is $d_E$; $A'$ lies on this circle of radius $r$. In other words, if $d_H$ is the hyperbolic metric and $d_H(O,A') = d_E(O,A)$, then we want to find the circle, with radius $r\in\R$ centered at $O$, that contains $A'$. From \todo{[LG] 9.1}, we obtain the hyperbolic metric
\[
	d_H(O,A') = \ln\frac{1 + r}{1 - r}
\]
where $r\in\R$ is the radius of the circle we wish to draw. We will call this new circle $\Delta$. Computing $r$, we find that
\[
...
Similarly, if $C$ is a point in $\R^2$ that appear to lie within $\Gamma$ (that is, the distance between $C$ and the center $O$ of $\Gamma$ is less than 1), then $C$ will simply be mapped to a point $C'$ on the line $\overline{OC}$ such that the distance between $O$ and $C'$ is less than the distance between $O$ and $C$. 
