...
Using the basic geometric fact that lines can be partitioned by a point into two sides \cite{hartshorne}, we then define the ray $\overrightarrow{AB}$ to be the point $A$ together with the collection of all points on the line defined by $A$ and $B$ that is on the same side of $A$ as $B$. 
