...
\begin{enumerate}
	...
	\item Finally, we find the points of intersection between this circle and this line: $(-0.85,-.51)$ and $(0.85,.51)$. Following the construction from Appendix \ref{appendixA}, we use the point of intersection that lies on the Euclidean ray $\overrightarrow{OA_2}$. Therefore we have that $g(A_2)' = (0.85, .51)$. 
\end{enumerate}

\noindent We now apply this same process to $g(f_X(A_2))$.

\begin{enumerate}
	\item Compute the Euclidean distance from the origin to point $g(f_X(A_2))$. In this case, we have $d_E((0,0),g(f_X(A_2))) = 0.946$.
	\item Next, we want to find the hyperbolic circle centered at $O = (0,0)$ whose Euclidean radius has a length equal to the Euclidean distance from $O$ to $g(f_X(A_2))'$. That is, we want the hyperbolic distance between $g(f_X(A_2))'$ and the origin to equal the Euclidean distance between $g(f_X(A_2))$ and the origin. We therefore let $d_H((0,0), g(f_X(A_2))') = d_E((0,0), g(f_X(A_2)))$. Thus, we have that
	\[
	...
	\item Finally, we find the points of intersection between this circle and this line: $(-0.434,-0.08)$ and $(0.434,0.08)$. Following the construction from Appendix \ref{appendixA}, we use the point of intersection that lies on the Euclidean ray $\overrightarrow{Of_X(A_2)}$. Therefore we have that $g(f_X(A_2))' = (0.434, 0.08)$. 
\end{enumerate}
...
Notice that $A$ and $B$, points in $\R^2$ on the ray $\overrightarrow{OA}$, remain on the ray $\overrightarrow{OA}$ after their translation to the \poincare disk. Further, in $\R^2$ we have the distance between points $A$ and $O$ is less than the distance between points $B$ and $O$. This relationship is preserved: in the \poincare disk we have that $A'$ is closer to the center $O$ than $B'$ is.

Similarly, if $C$ is a point in $\R^2$ that appears to lie within $\Gamma_{\R^2}$ (that is, the distance between $C$ and the center $O$ of $\Gamma_{\R^2}$ is less than 1), then $C$ will simply be mapped to a point $C'$ on the line segment $\overline{OC}$ such that the distance between $O$ and $C'$ is less than the distance between $O$ and $C$. 
