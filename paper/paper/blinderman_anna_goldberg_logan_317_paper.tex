...
Inspired by Euclidean geometry, we begin with the notion that both types of reflections should be of order two. That is, we want the composition of a transformation upon itself to yield the identity transformation. We first establish two lemmas. 
...
MOVE THIS SOMEWHERE Further, in order to distinguish line reflections given by lines through the end $\specialend$ and those not through $\specialend$, Guggenheimer takes the field in question and defines $F_*$ to be the extension of $F$ by $\infty$ as usual. While we can certainly do this in $\R$, it is unclear how to do this in an arbitrary field. 
