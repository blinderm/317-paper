...
We now go forth to visualize $f_X(A_2)$. First, we take $A_2$ and $f_X(A_2)$ and map them to their corresponding\footnote{We have that $a + ib \in \C$ corresponds to the point $(a\ttc b)$ in $\R^2$. Conversely, we say that $(a\ttc b)$ in $\R^2$ corresponds to $a+ib$ in $\C$.} points in $\R^2$, which we will denote by  $A_{2_{\R^2}}$ and $f_X(A_2)_{\R^2}$. In particular, we let $A_{2_{\R^2}} = (3,5)$ and $f_X(A_2)_{\R^2} = (27/29, 5/29)$. Now, given the point $A_{2_{\R^2}}$ in $\R^2$, we apply the methodology from Appendix \ref{appendixA} to construct the accompanying point $A_{2_{\R^2}}'$ in the \poincare disk. 

\begin{enumerate}
	\item Compute the Euclidean distance from the origin to point $A_{2_{\R^2}}$. In this case, we have $d_E((0,0),A_{2_{\R^2}}) = \sqrt{34}$.
	\item Next, we want to find the radius of the circle that is equal to the Euclidean distance in hyperbolic distance. In particular, let $d_H(O,A_{2_{\R^2}}') = d_E(O,A_{2_{\R^2}})$. Therefore, we have that
	\[
		d_H(O,A_{2_{\R^2}}') = \sqrt{34} = \ln\frac{1 + r}{1 - r}.
	\]
	...
	\item Finally, we find the points of intersection between the circle and the line: $\{(-0.85,-.51),(0.85,.51)\}$. Since $A$ is positive in both coordinates, we want the point of intersection that is also positive in both coordinates. Therefore $A' = (0.85, .51)$. \todo{[LG] why are we just allowed to flip this}
\end{enumerate}

The point $A_{2_\R}'$ is also in $\R^2$ but is the point $A_2$ viewed\footnote{By viewed, we mean that $A_{2_\R}'$ is $A_{2_\R}$ in the \poincare model.} from the \poincare disk. \todo{[LG] not sure what this means/does} We apply this same process to $f_X(A_2)_\R$.
\end{example}
