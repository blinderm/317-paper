...
We now begin the process of visualizing $f_X(A_2)$. First, we take $A_2$ and $f_X(A_2)$ and map them to their corresponding \todo{[LG] comment on this ``correspondance"} points in $\R^2$. In particular, $A_{2_\R} = (3,5)$ and $f_X(A_2)_\R = (27/29, 5/29)$. Now, given the point $A_{2_\R}$ in $\R^2$, we apply the methodology from Appendix \ref{appendixA} to construct the accompanying point $A_{2_\R}'$ in the \poincare disk. \todo{The point $A_{2_\R}'$ is also in $\R^2$ but is the point $A_2$ viewed\footnote{By viewed, we mean that $A_{2_\R}'$ is $A_{2_\R}$ in the \poincare model.} from the \poincare disk. [LG] not sure what this sentence adds}We apply this same process to $f_X(A_2)_\R$.
\end{example}
...
Let $\Gamma_{F^2}$ be a \poincare disk. \todo{introduce the notation? also the diagram and this thing are inconsistent -- not sure which you want [LG]} Suppose that we wish to translate the Euclidean point $A$, pictured arbitrarily below, to its equivalent point $A'$ in the \poincare disk.
