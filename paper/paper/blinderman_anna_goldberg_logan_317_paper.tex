...
To understand hyperbolic geometry we must first consider the five postulates that axiomatize Euclidean geometry. The first four are as follows \cite{hartshorne} \todo{[AB][LG] is this the right place for the citation?}:
...
\begin{enumerate}[leftmargin = 4em, itemsep=-1em]
	\setcounter{enumi}{4}
	\item Given a line and a point on that line, at most one line parallel to the given line can be drawn through that line.
\end{enumerate}
is replaced with its logical negation: ``Through an exterior point to a straight line we can construct an infinite number of parallels to that straight line." \cite{euclids_fifth} \todo{[LG] deal with the whole infinitely many thing}. As demonstrated below, line $m$ is the unique line through $P$ parallel to $l$ in Euclidean geometry while lines $n_1$ and $n_2$ through $P$ are parallel to $l$ in hyperbolic geometry.
...
\end{tabular}
\end{center}
In the Euclidean model, $m$ is the unique line through $P$ and parallel to $l$. On the other hand, in the hyperbolic model we have that $n_1$ and $n_2$ are distinct lines passing through $P$ and parallel to $l$. Further, there are infinitely many distinct lines with these two properties. \todo{[LG] deal with the whole infinitely many thing}

\todo{[LG] use Karen's whole $\Gamma_{F^2}$ notation thing?}
...
\noindent\todo{[LG] draw on stuff you put in the intro to make this clearer}
...
Returning to our linear fractional transformations, we begin with a notion inspired by Euclidean geometry: that both point reflections and line reflections should be of order two. That is, we want the composition of a transformation upon itself to yield the identity transformation. We first establish two lemmas. 

\noindent \todo{[LG] i put these things as lemmas, but feel free to move them to appendix c}

\begin{lemma} 
	\todo{[LG][AB] phrasing?} The composition of linear fractional transformation corresponds to the multiplication of their respective matrix encodings.
\end{lemma}

\begin{proof} 
	Suppose that we have two linear fractional transformations $f_1$ and $f_2$ given by $f(z) = \frac{a_1z+b_1}{c_1z+d_2}$ and $f_2(z) = \frac{a_2z+b_2}{c_2z+d_2}$ for each $z \in \C$. We then have \todo{[LG] BY CONVENTION??} that their respective matrix encodings are 
\[ M_{f_1} = \lftmat{a_1}{b_1}{c_1}{d_1} \text{ and } M_{f_2} =  \lftmat{a_2}{b_2}{c_2}{d_2}. \] 
Now, let $z \in \C$. We have that 
\begin{align*}
f_1(f_2(z)) & = \frac{a_1(\frac{a_2z+b_2}{c_2z+d_2})+b_1}{c_1(\frac{a_2z+b_2}{c_2z+d_2})+d_1} \\[1em]
& = \frac{\frac{a_1(a_2z+b_2)+b_1(c_2z+d_2)}{c_2z+d_2}}{\frac{c_1(a_2z+b_2)+d_1(c_2z+d_2)}{c_2z+d_2}} \\[1em]
& = \frac{a_1(a_2z+b_2)+b_1(c_2z+d_2)}{c_1(a_2z+b_2)+d_1(c_2z+d_2)} \\[1em]
& = \frac{(a_1a_2+b_1c_2)z + (a_1b_2+b_1d_2)}{(c_1a_2+d_1c_2)z+(c_1b_2+d_1d_2)}
\end{align*}
On the other hand, we have that 
\begin{align*}
M_{f_1}M_{f_2} & = \lftmat{a_1}{b_1}{c_1}{d_1}\lftmat{a_2}{b_2}{c_2}{d_2}  \\[1em]
& = \lftmat{a_1a_2+b_1c_2}{a_1b_2+b_1d_2}{c_1a_2+d_1c_2}{c_1b_2+d_1d_2}
\end{align*}
Notice that this resultant matrix $M_{f_1}M_{f_2}$ encodes the linear fractional transformation given by $f_1 \circ f_2$, the composition of $f_1$ upon $f_2$ \todo{[LG] it is $f_1$ upon $f_2$, right..?}. 
\end{proof} 
...
\begin{proof}
	Because $f$ is of order two, by definition we must have that $f \circ f$ is the identity function. In terms of matrices, by Lemma 3.1 we must then have that $M_f^2$ yields a member of the coset \todo{[LG] do we need to cit something about cosets here again?} containing the identity matrix $I$. That is, we must have that $M_f^2 = \lambda I$ for some $\lambda \in F$ .
\end{proof}
...
\begin{theorem}
If $M$ is a matrix encoding some linear fractional transformation $f$, then $f$ is of order two under matrix multiplication if and only if $\Tr(M) = 0$ or $M = \lambda I$ for some $\lambda \in F$.\footnote{That is, $\lambda I$ is some member of the coset \todo{[LG] $PGL(2\ttc F)$, cite appendix c?}containing the identity.}
\end{theorem}
...
Now, below are our diagrams.

\iffalse
\begin{center}
\includegraphics[width=165mm]{../images/t1_over_x.png} \\
Triangle $T_1$ reflected over point $X$ 
\[\]
\includegraphics[width=165mm]{../images/t1_over_y.png} \\
Triangle $T_1$ reflected over point $Y$  
\[\]
\includegraphics[width=165mm]{../images/t2_over_x.png} \\
Triangle $T_2$ reflected over point $X$
\[\]
\includegraphics[width=165mm]{../images/t2_over_y.png} \\
