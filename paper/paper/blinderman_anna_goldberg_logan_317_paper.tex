...
TODO: use Karen's whole $\Gamma_{F^2}$ or whatever thing
...
\end{equation}
under function composition. Remarkably, Guggenheimer does not say anything about the case when $z = -\frac{d}{c}$. However, we believe this case to correspond to the output $\specialend$, which Guggengheimer discusses when distinguishing types of lines defining line reflections.
...
\begin{lemma} 
	TODO: PHRASING? The composition of linear fractional transformation corresponds to the multiplication of their respective matrix encodings.
\end{lemma}

\begin{proof} 
	Suppose that we have two linear fractional transformations $f_1$ and $f_2$ given by $f(z) = \frac{a_1z+b_1}{c_1z+d_2}$ and $f_2(z) = \frac{a_2z+b_2}{c_2z+d_2}$ for each $z \in \C$. We then have BY CONVENTION?? that their respective matrix encodings are 
...
\end{align*}
Notice that this resultant matrix $M_{f_1}M_{f_2}$ encodes the linear fractional transformation given by $f_1 \circ f_2$, the composition of $f_1$ upon $f_2$ TODO: ORDER OF F ONE AND F TWO THERE?. 
\end{proof} 
...
\begin{proof}
	Because $f$ is of order two, by definition we must have that $f \circ f$ is the identity function. In terms of matrices, by Lemma 3.1 we must then have that $M_f^2$ yields a member of the coset TODO: DO WE NEED TO CITE SOMETHING TO BE TLAKING ABOUT COSETS? containing the identity matrix $I$. That is, we must have that $M_f^2 = \lambda I$ for some $\lambda \in F$ .
\end{proof}
...
Recall further that Guggenheimer distinguishes between line reflections defined by two different types of lines: those passing through the end $\specialend$ and those not passing through the end $\specialend$. Although we suspect that for a linear fractional transformation $f$ given by $z \mapsto \frac{az+b}{cz+d}$, the end $\specialend$ corresponds to the expression $f(\frac{-d}{c})$ (that is, division by zero), this is not addressed by Guggenheimer and thus we avoid alksdjnflaksjdfnalksjdfnalskdjnlaskdj n

To be clear, in our diagrams it will always appear that our hyperbolic lines can be extended to pass through two ends (i.e. touch the edge of the \poincare disk in two places). However, these are the general ends of the \poincare disk -- those mapped from the parabola $x = y^2$ in $\R^2$. Because we explicitly avoided division by zero aksjdfbalkjdfn 

However, by the distance metric in the \poincare disk CITE WHATEVER APPENDIX THAT IS we now that are lines  we avoided line reflections (and thus their corresponding lines) that involve 
...
%\cite{Ahl} is a classic reference text.
\nocite{*}
