...
\begin{enumerate}
	...
	\item Next, we want to find the hyperbolic circle centered at $O = (0,0)$ whose Euclidean radius has a length equal to the Euclidean distance from $O$ to $g(A_2)'$. For this mapping, we want $g(A_2)'$ to be the same hyperbolic distance away from the origin as $g(A_2)$ is away from the origin in Euclidean distance. In particular, let $d_H((0,0), g(A_2)') = d_E((0,0), g(A_2))$. Therefore, we have that
	\[
...
\begin{enumerate}
	...
	\item Next, we want to find the hyperbolic circle centered at $O = (0,0)$ whose Euclidean radius has a length equal to the Euclidean distance from $O$ to $g(f_X(A_2))'$.  For this mapping, we want $g(f_X(A_2))'$ to be the same hyperbolic distance away from the origin as $g(f_X(A_2))$ is away from the origin in Euclidean distance.  In particular, let $d_H((0,0), g(f_X(A_2))') = d_E((0,0), g(f_X(A_2)))$. Therefore, we have that
	\[
