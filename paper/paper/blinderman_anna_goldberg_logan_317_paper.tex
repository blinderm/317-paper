...
\begin{center}
	...
	(-2.15,.48)
	\draw (-1.95, -1) -- (1.95, 1) node[below left=.2cm] {};
	\draw (-2.3,.35)  -- (-2.26,.5);
	\draw (-2.3,.35)  -- (-2.18,.32);
...
We now discuss hyperbolic line segments. When we refer to hyperbolic line segments, we mean line segments of hyperbolic lines. In our above diagram, $\overline{GH}$ and $\overline{EF}$ are hyperbolic line segments lying on the hyperbolic lines $AB$ and $LK$ respectively. In general, if we have a hyperbolic line $XY$ and two points $Z$ and $W$ lying on $XY$ in the \poincare disk, then $\overline{ZW}$ is defined to be the set of all points on $XY$ between (and including) $Z$ and $W$.
...
We first work through a concrete example -- reflecting the point $B_1 = 1+0i$ over the point $X = (2, -1)$ -- to demonstrate the process and calculations behind our reflections.
...
Notice that for simplicity, we chose $\lambda = 1$. Now, we know by (2) that for all $z \in \C$, $f_X$ is given by $f_X(z) = \frac{-z+2}{-z+1}$. Therefore we have that $f_X(B_1) = \frac{27+5i}{29}$. 

After we have computed $f_X(B_1)$, we start the visualization process. First, we take $B_1$ and $f_X(B_1)$ and map them to their corresponding points in $\R^2$. In particular, let $B_{1_\R} = (1,0)$ and $f_X(B_1)_\R = (27/29, 5/29)$. Now, given the point $A_{2_\R}$ in $\R^2$, we apply the methodology from Appendix \ref{appendixA} to construct the accompanying point $A_{2_\R}'$ in the \poincare disk. 

\begin{enumerate}
	\item In Geogebra, open the hyperbolic worksheet.
	\item In the input bar at the bottom of the screen, type $O = (0,0)$ and press enter to construct the center of the \poincare disk.
	\item Again in the input bar, type the point $A_2 = (3,5)$ and press enter. This will construct the Euclidean point $A_2$, so it will be placed at the coordinates $(3,5)$. 
	\item Navigating to the toolbars at the top of the window, click on the distance tool and select points $O$ and $A$ with the mouse to compute the distance between $O$ and $A$. The result should return $distanceOA2 = 5.83095$.
	\item Once computed, navigate to the tool called `Hyperbolic Circle w Given Radius'. Select the point $O$ and type into the popup window `distanceOA2'. This step should construct a hyperbolic circle with radius distanceOA2 and center $O$.
	\item Navigate to the 
\end{enumerate}
Using the hyperbolic worksheet in Geogebra, we begin by constructing the point $O = (0,0)$ to establish the origin of the \poincare disk. Then constructing the point 




The point $A_{2_\R}'$ is also in $\R^2$ but is the point $A_2$ viewed\footnote{By viewed, we mean that $A_{2_\R}'$ is $A_{2_\R}$ in the \poincare model.} from the \poincare disk. We apply this same process to $f_X(A_2)_\R$.
\end{example}
