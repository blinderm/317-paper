...
\begin{proof} 
	Appeal to Theorem \ref{thm:function_correspondence}.		
\end{proof}	
...
\begin{proof}
	Because $f$ is of order two, by definition we must have that $f \circ f$ is the identity function. In terms of matrices, by Lemma 3.1 we must then have that $M_f^2$ yields the identity of $PGL(2\ttc F)$. That is, we must have that $M_f^2 = \lambda I$ for some $\lambda \in F$.
\end{proof}
...
\begin{theorem}
If $M$ is a matrix encoding some linear fractional transformation $f$, then $f$ is of order two under matrix multiplication if and only if $\Tr(M) = 0$ or $M = \lambda I$ for some $\lambda \in F$.\footnote{That is, $\lambda I$ is some representative of a of coset $PGL(2\ttc F)$ containing the identity. See Appendix \ref{appendixC}.}
\end{theorem}
